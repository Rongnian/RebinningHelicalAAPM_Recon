...
cfg = CollectReconCfg([FilePath,'/',PathInfo(1).name]);
...
proj = proj(end:-1:1,end:-1:1,:); % Because the differences in GE and SIEMENS definition, we change it to GE standard.

%% Rescale the projection
proj = proj * cfg.RescaleSlope + cfg.RescaleIntercept;
end
